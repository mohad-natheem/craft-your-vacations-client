...
import type { NavLink, FooterAction } from '@/app/types'
...
export function Footer({
 ...
 const copyright = copyrightText ?? `© ${year} CraftVacations. All rights reserved.`
   ...
   <footer className={`bg-surface-low border-t border-outline ${className}`}>
     <div className="mx-auto max-w-7xl px-6 py-16">
       <div className="grid grid-cols-1 md:grid-cols-4 gap-12">
         ...
         {linkGroups.map((group) => (
           <div key={group.heading} className="flex flex-col gap-4">
             <h4 className="text-label-md text-text-subtle">{group.heading}</h4>
       ...
       <div className="border-t border-outline mt-12 pt-8">
         <p className="text-body-sm text-text-subtle text-center">{copyright}</p>
       </div>
     </div>
   </footer>
 )
